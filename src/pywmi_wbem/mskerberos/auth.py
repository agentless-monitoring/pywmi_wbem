    ...
    if response.headers.get("content-type") != None:
      if response.headers["content-type"].startswith("multipart/encrypted"):
        ...
        if options.get("protocol") is not None and options["protocol"] == "application/HTTP-Kerberos-session-encrypted":
          ...
          for part in re.split(re_multipart.encode(), response.content):
            if part == b'':
              continue
          ...
          response.headers[b"Content-Type"]=original_values[b"type"]+b"; charset="+original_values[b"charset"]
          response.headers[b"Content-Length"]=len(decrypted)
