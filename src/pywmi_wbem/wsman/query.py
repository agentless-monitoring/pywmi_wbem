...
class States():
  ...
  CREATE_RESPONSE = {"name": "CreateResponse",
                     "action": "http://schemas.xmlsoap.org/ws/2004/09/transfer/CreateResponse"}
  COMMAND_RESPONSE = {"name": "CommandResponse",
                      "action": "http://schemas.microsoft.com/wbem/wsman/1/windows/shell/CommandResponse"}
  FETCH_OUTPUT = {"name": "FetchOutput", "action": "Fetch"}
  RECEIVE_RESPONSE = {"name": "ReceiveResponse",
                      "action": "http://schemas.microsoft.com/wbem/wsman/1/windows/shell/ReceiveResponse"}
  RECEIVE_DONE = {"name": "RECEIVE_DONE", "action": "ReceiveDone"}
  SIGNAL_RESPONSE = {"name": "SIGNAL_RESPONSE",
                     "action": "http://schemas.microsoft.com/wbem/wsman/1/windows/shell/SignalResponse"}
  DELETE_SHELL = {"name": "DeleteShell", "action": "http://schemas.xmlsoap.org/ws/2004/09/transfer/DeleteResponse"}
  # WQL States
  ENUMERATE_RESPONSE = {"name": "EnumareResponse",
...
class WSMAN_Constants:
  ...
  SELECTOR_PATH = "Envelope.Body.{%s}ResourceCreated.{%s}ReferenceParameters.%s.%s" % (
    TRANSFER, WSA, SELECTOR_SET_TAG, SELECTOR_TAG)
...
class WSMan():
  ...
  def generate_invoke_action_query(self, to, uuid, obj, method, selectorset, params=None, namespace="root/cimv2"):
    ...
    for i in selectorset:
      ...
      selector_element = ET.SubElement(selectorset_element,
                                       "{http://schemas.dmtf.org/wbem/wsman/1/wsman.xsd}Selector")
      selector_element.attrib["Name"] = key
      selector_element.text = str(value)

    root = self.generate_message(resource_uri + "/" + method, to, resource_uri, uuid, input_element,
                                 sub_header=selectorset_element)
  ...
  def wql_parse_pull_response(self):
    ...
    item_fragment_path = objectify.ObjectPath(
      "Envelope.Body.{%s}PullResponse.Items.{%s}XmlFragment" % (WSMAN_Constants.WSEN, WSMAN_Constants.WSMAN))
  ...
  def create_shell(self):
    ...
    root = self.generate_message(action, self.host, WSMAN_Constants.CMD, uuid.uuid4(), sub_body=sub_body,
                                 sub_header=self.get_shell_sub_header())
    msg = ET.tostring(root)
    create_shell_response = self.session.post(self.host, auth=self.auth, data=msg, headers=self.headers,
  ...
  def shell_execute_command(self):
    ...
    # send command
    root = self.generate_message(WSMAN_Constants.COMMAND_ACTION, self.host, WSMAN_Constants.CMD, uuid.uuid4(),
                                 commandLine, sub_header=sub_header)
    command_msg = ET.tostring(root)
    command_response = self.session.post(self.host, auth=self.auth, data=command_msg, headers=self.headers,
                                         timeout=self.timeout)
  ...
  def shell_fetch_output(self, sequence):
    ...
    root = self.generate_message(WSMAN_Constants.RECEIVE_ACTION, self.host, WSMAN_Constants.CMD, uuid.uuid4(),
                                 sub_body, self.get_shell_sub_header())
  ...
  def delete_shell(self):
    root = self.generate_message(WSMAN_Constants.DELETE_ACTION, self.host, WSMAN_Constants.CMD, uuid.uuid4(),
                                 sub_header=self.get_shell_sub_header())
  ...
  def shell_terminate_signal(self):
    ...
    root = self.generate_message(WSMAN_Constants.SIGNAL_ACTION, self.host, WSMAN_Constants.CMD, uuid.uuid4(),
                                 sub_body=sub_body, sub_header=self.get_shell_sub_header())
